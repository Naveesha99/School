...
module.exports.createCleaner = async (req, res, next) => {
    try {
        ...
        const createCleaner = await Student.create({ CleanerID, CleanerName, CleanerContact, createdAt });
        res
          .status(201)
...
module.exports.viewCleaner = async (req, res, next) => {
  ...
  module.exports.updateCleaner = async (req, res) => {
    try {
      ...
      res.status(200).json({ message: "Cleaner updated successfully", success: true, updatedCleaner: student });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: "Internal server error" });
    }
