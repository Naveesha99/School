...
module.exports.createTeacher = async (req, res, next) => {
    try {
        const {TeacherID, TeacherName, TeacherEmail, Subject ,createdAt  } = req.body;
        const teacher = await Teacher.findOne({ TeacherID });
        if (teacher) {
          return res.status(400).json({ message: "Teacher already exists" });
        }
        const createTeacher = await Teacher.create({ TeacherID, TeacherName, TeacherEmail, Subject ,createdAt });
        res
          .status(201)
          .json({ message: "Created the Teacher log successfully", success: true, createTeacher });
      } catch (error) {
        console.error(error);
      }
};
...
module.exports.viewTeacher = async (req, res, next) => {
  ...
  module.exports.updateTeacher = async (req, res) => {
    try {
      const { TeacherID } = req.params;
      const {  TeacherName, TeacherEmail, Subject ,createdAt } = req.body;
